...
This extension deliberately uses numeric `{prefix}{n}` and `{prefix}ref-{n}` IDs rather than cmark's label-derived IDs, and does not add a `user-content-` clobber prefix. It also omits `class="footnote-ref"` on `<sup>` and `data-footnote-backref-idx` on backrefs. Footnote definitions may interrupt a paragraph. References inside a definition's own content are numbered in lex order, so a nested reference can receive a lower number than a later main-text reference, unlike cmark-gfm's main-text-first ordering. A definition inside a blockquote or list item leaves an empty container in the document flow while its note is hoisted to the footnotes section. A definition directly above a setext underline can be absorbed into the setext heading because of marked's block-tokenizer precedence.
